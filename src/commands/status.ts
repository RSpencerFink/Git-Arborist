    ...
    try {
      ...
      if (branchStatus.dirty) {
        ...
        if (branchStatus.staged > 0)
          parts.push(c.green(`+${branchStatus.staged}`));
        if (branchStatus.modified > 0)
          parts.push(c.yellow(`~${branchStatus.modified}`));
        if (branchStatus.untracked > 0)
          parts.push(c.dim(`?${branchStatus.untracked}`));
      ...
      if (branchStatus.ahead > 0)
        syncParts.push(c.green(`↑${branchStatus.ahead}`));
      if (branchStatus.behind > 0)
        syncParts.push(c.red(`↓${branchStatus.behind}`));
