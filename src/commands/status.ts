...
export async function statusJson(ctx: ArboristContext, _args: string[]): Promise<StatusJsonItem[]> {
...
export async function status(ctx: ArboristContext, _args: string[]): Promise<void> {
  const worktrees = await getWorktrees(ctx);
  const currentWt = await getCurrentWorktree(ctx);
