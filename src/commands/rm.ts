...
export async function rmJson(
  ctx: GwContext,
  args: string[],
): Promise<RmJsonResult> {
  const parsed = parseRmArgs(args);

  if (!parsed.name) {
    throw new Error(
      "Worktree name required in --json mode. Usage: gw rm <name> --json",
    );
  }
...
export async function rm(ctx: GwContext, args: string[]): Promise<void> {
  const parsed = parseRmArgs(args);
