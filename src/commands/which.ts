...
export async function whichJson(ctx: ArboristContext, _args: string[]): Promise<WhichJsonResult> {
  ...
  for (const wt of nonBare) {
    const isCurrent = wt.path === currentPath || currentPath.startsWith(`${wt.path}/`);
...
export async function which(ctx: ArboristContext, _args: string[]): Promise<void> {
  const worktrees = await getWorktrees(ctx);
  const nonBare = worktrees.filter((wt) => !wt.isBare);
  const currentPath = process.cwd();
