...
interface CommandHandler {
  ...
  runJson?: (ctx: ArboristContext, args: string[]) => Promise<unknown>;
...
const commands: Record<string, () => Promise<CommandHandler>> = {
  ...
  clone: async () => ({
    run: async (_ctx: ArboristContext, args: string[]) => {
...
export async function dispatch(argv: string[]): Promise<void> {
  ...
  if (command === 'completions') {
    ...
    if (!shell) {
      throw new Error('Shell argument required. Usage: arb completions <zsh|bash|fish>');
    }
  ...
  if (command === 'shell-init') {
    const shell = commandArgs[0];
    if (!shell) {
      throw new Error('Shell argument required. Usage: arb shell-init <zsh|bash|fish>');
    }
  ...
  if (!loader) {
    ...
    log.dim('Run arb --help to see available commands.');
    process.exit(1);
  }
