...
export async function removeWorktree(path: string, force?: boolean, cwd?: string): Promise<void> {
...
export async function getDefaultBranch(cwd?: string): Promise<string> {
  ...
  const result = await exec(['git', 'symbolic-ref', 'refs/remotes/origin/HEAD', '--short'], {
    cwd,
  });
  ...
  for (const branch of ['main', 'master']) {
    const check = await exec(['git', 'rev-parse', '--verify', `refs/heads/${branch}`], { cwd });
...
export async function getBranchStatus(branch: string, cwd?: string): Promise<BranchStatus> {
...
export async function branchExists(branch: string, cwd?: string): Promise<boolean> {
  const result = await exec(['git', 'rev-parse', '--verify', `refs/heads/${branch}`], { cwd });
  return result.exitCode === 0;
}

export async function isMerged(branch: string, into: string, cwd?: string): Promise<boolean> {
  const result = await exec(['git', 'merge-base', '--is-ancestor', branch, into], { cwd });
  return result.exitCode === 0;
}
